...
- (UIView *)menuView {
    if (!_menuView) {
        ...
        UIButton *selectBtn = (UIButton *)[_menuView viewWithTag:_selectedIndex+100];
...
- (void)setSelectedIndex:(NSUInteger)index {
    if (index != _selectedIndex) {
        UIButton *origin = (UIButton *)[_menuView viewWithTag:_selectedIndex+100];
        if ([origin isKindOfClass:[UIButton class]]) {
            [origin setTitleColor:self.unselectedColor forState:UIControlStateNormal];
        }
        
        UIButton *button = (UIButton *)[_menuView viewWithTag:index+100];
        [button setTitleColor:self.selectedColor forState:UIControlStateNormal];
        _lineView.frame = CGRectMake(button.frame.origin.x, button.frame.size.height-1, button.frame.size.width, 1);

        _selectedIndex = index;
        _lastSelectedIndex = index;
        
        if ([self.delegate respondsToSelector:@selector(menuView:didSelectedIndex:)]) {
            [self.delegate menuView:self didSelectedIndex:index];
        }
    }
}
